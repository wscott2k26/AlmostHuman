...
serve(async (req) => {
  try {
    ...
    if (preview) {
      text = PREVIEWS[voiceId] || PREVIEWS['female-adult'];
    } else {
      const aiId = String(body.ai_entity_id || '').trim();
      if (!aiId || !text) return Response.json({ error: 'ai_entity_id and text required', code: 'INVALID_REQUEST' }, { status: 400 });
      const [ai, settingsRows] = await Promise.all([
        ctx.entities.AIEntity.get(aiId),
        ctx.entities.AppSettings.list('-created_date', 1).catch(() => []),
      ]);
      if (!ai || ai.created_by_id !== ctx.user.id || ai.archived) return Response.json({ error: 'Not found', code: 'NOT_FOUND' }, { status: 404 });
      ...
      voiceId = normalizePublicVoiceId(ai.voice_id || voiceId);
    }

    const generated = await generateNeuralSpeech({ text, voiceId, stageLabel, requestId });
    ...
    return Response.json({ error: safeError(error), code: String((error as any)?.code || 'VOICE_FAILED') }, { status: statusOf(error) });
  }
});
